...
class TensorCoreStateNode : public StateNode {
 public:
  ...
  /*! \brief Flag to indicate its a WGMMA intrin group */
  bool is_hopper;
...
TensorCoreState::TensorCoreState(TensorCoreIntrinGroup intrin_group,
  ...
  node->is_hopper = support::StartsWith(intrin_group.compute_intrin, "hopper_sync");
 ...
 * intrinsics.
 */
class MultiLevelTilingTensorCoreHopperNode : public MultiLevelTilingNode {
 private:
  ...
  // Subrule: Add read cache for wgmma
  // Basically same with MultiLevelTilingNode::AddReadReuse, but change CacheRead + ComputeAt to
  // ReadAt
  inline std::vector<State> WGMMAAddReadReuse(TensorCoreState state) const;
  ...
  // Subrule: split loop for wgmma using sample partitioned tile
  inline std::pair<Array<tir::ExprRV>, Array<tir::LoopRV>> WGMMASplitLoop(const Schedule& sch,
                                                                        BlockRV block, LoopRV loop,
                                                                        int n_tiles,
                                                                        int partition_pos,
                                                                        int innerpart_factor) const;
  // Subrule: tile loop nest for wgmma
  // Basically same with MultiLevelTilingNode::TileLoopNest, but change SamplePerfectTile to
  // SamplePartitionedTile
  inline std::vector<State> WGMMATileLoopNest(TensorCoreState state) const;
  ...
  ScheduleRule Clone() const final {
    ObjectPtr<MultiLevelTilingTensorCoreHopperNode> n =
        make_object<MultiLevelTilingTensorCoreHopperNode>(*this);
 ...
 public:
  ...
  static constexpr const char* _type_key = "meta_schedule.MultiLevelTilingTensorCoreHopper";
  TVM_DECLARE_FINAL_OBJECT_INFO(MultiLevelTilingTensorCoreHopperNode, MultiLevelTilingNode);

 private:
};

// Entry of the mega rule; Inherited from ScheduleRuleNode
Array<Schedule> MultiLevelTilingTensorCoreHopperNode::Apply(const Schedule& sch,
...
std::vector<State> MultiLevelTilingTensorCoreHopperNode::ApplySubRules(std::vector<State> states) {
  ...
  states = SubRule(std::move(states), [&](State state) {
    ...
    return tc_state->is_hopper ? WGMMATileLoopNest(tc_state) : TileLoopNest(state, 2);
  });
  states = SubRule(std::move(states), [&](State state) {
  ...
  states = SubRule(std::move(states), [&](State state) {
    TensorCoreState tc_state = Downcast<TensorCoreState>(state);
    return tc_state->is_hopper ? WGMMAAddReadReuse(tc_state) : AddReadReuse(state);
  });
  states = SubRule(std::move(states), [&](State state) {
...
void MultiLevelTilingTensorCoreHopperNode::TileAndAnnotateTensorize(
...
std::vector<State> MultiLevelTilingTensorCoreHopperNode::WGMMAAddReadReuse(TensorCoreState state) const {
...
std::pair<Array<tir::ExprRV>, Array<tir::LoopRV>> MultiLevelTilingTensorCoreHopperNode::WGMMASplitLoop(
...
std::vector<State> MultiLevelTilingTensorCoreHopperNode::WGMMATileLoopNest(TensorCoreState state) const {
  Schedule& sch = state->sch;
  const BlockRV& block_rv = state->block_rv;
  // Step 1. Assuming trivial binding, pair the loops and their iter-var-types
  Array<LoopRV> loops = sch->GetLoops(block_rv);
  if (!(loops.size() == 3 || !state->is_hopper)) {
    LOG(DEBUG) << "The WGMMA tensor core only supports SSR loops now";
    return {};
  }
...
std::vector<State> MultiLevelTilingTensorCoreHopperNode::TransformIntermediateOutputLayout(
  ...
  // Get the shape of the wgmma accumulator
...
std::vector<State> MultiLevelTilingTensorCoreHopperNode::AddWriteReuseTensorCore(
  ...
  Schedule& sch = state->sch;
  auto cache_write = sch->CacheWrite(state->block_rv, 0, "wgmma.accumulator");
  ...
  //
  // After adding cache write for wgmma.accumulator, we will have
...
std::vector<State> MultiLevelTilingTensorCoreHopperNode::AddReadReuseTensorCore(
    TensorCoreState state) const {
  ...
  auto f_tensorize_load = [&](int read_index, String scope, String intrin_name) {
    ...
    String permuted_layout_annotate_value =
        state->is_hopper ? std::string("s2l_") + std::string(read_index == 0 ? "A" : "B") : "";
    TileAndAnnotateTensorize(&sch, cache_read, intrin_name, permuted_layout_annotate_value);
  };
  f_tensorize_load(0, state->is_hopper ? "m16n8k8.matrixA" : "wgmma.matrix_a",
                   state->intrin_group.load_a_intrin);
  f_tensorize_load(1, state->is_hopper ? "m16n8k8.matrixB" : "wgmma.matrix_b",
...
std::vector<State> MultiLevelTilingTensorCoreHopperNode::AddSoftwarePipeline(
    TensorCoreState state) const {
  ...
  sch->Annotate(state->tiles[r_indices_[1]].back(), tir::attr::software_pipeline_order,
                Array<Integer>{0, 1, 2});
  if (state->is_hopper && state->use_async) {
...
Optional<LoopRV> MultiLevelTilingTensorCoreHopperNode::TransformWithTensorIntrin(
...
inline std::vector<State> MultiLevelTilingTensorCoreHopperNode::TransformForTensorization(
    TensorCoreState state) const {
...
ScheduleRule ScheduleRule::MultiLevelTilingTensorCore(
  ...
  auto node = MultiLevelTilingInitCommon<MultiLevelTilingTensorCoreHopperNode>(
...
TVM_REGISTER_NODE_TYPE(MultiLevelTilingTensorCoreHopperNode);
TVM_REGISTER_GLOBAL("meta_schedule.ScheduleRuleMultiLevelTilingTensorCoreHopper")
