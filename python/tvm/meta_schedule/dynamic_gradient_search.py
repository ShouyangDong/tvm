...
def get_possible_tiling_decisions(
    tile_product: int, num_tiles: int, max_inntermost_factor=64
) -> List[Tuple]:
...
class DynamicGradientSearchTuner:
    ...
    def __init__(
        ...
        self.model = CostModel.create("xgb", num_tuning_cores=os.cpu_count(), tree_method="auto")
        ...
        self.postprocs = ms.Postproc.create("cuda")
        self.target = target
        ...
        self.instruction_decsion_key = {}

    def get_decisions(sch: Schedule) -> dict:
        ...
        for inst, decision in sch.trace.decisions.items():
            if inst.kind.name == "SamplePerfectTile":
                ...
                inst_dec_tag: str = get_parameter_name(inst, decision)
        ...
        for inst, decisions in untuned_sch.trace.decisions.items():
            if inst.kind.name == "SamplePerfectTile":
                ...
                if decision_key not in decision_lookup:
                    ...
                    possible_decisions = get_possible_tiling_decisions(
                        total_loop_iters, n_splits, max_innermost_factor
                    )
                ...
                inst_dec_tag: str = get_parameter_name(inst, decisions)
    ...
    def create_schedule_from_trace(sch, trace, postprocs):
        """Create a post processd Schedule from a Schedule and Trace.

        Prameters
        ---------
        sch : tvm.ir.Schedule
            The Schedule of the workload.
        ...
        sch : tvm.schedule.Schedule | None
            The created schedule or None if post procssing fails
        """
    ...
    def dgd_move(
        ...
        while index_slide < len(sorted_indices) and not next_base:
            ...
            for record in selected_records:
                ...
                postproc_schs.append(
                    create_schedule_from_trace(candidate.sch, record.trace, self.postprocs)
                )

            # get the slide window inputs
            slide_window_inputs = [
                ms.builder.BuilderInput(postproc_sch.mod, self.target)
                for postproc_sch in postproc_schs
            ]
    ...
    def dynamic_gradient_search(self):
        ...
        raw_states = self.sample_init_population(
            self.context.search_strategy, self.init_population_size
        )
        ...
        builder_inputs = [ms.builder.BuilderInput(state.mod, self.target) for state in topk_states]
        builder_results = self.builder.build(builder_inputs)
        valid_indices = []
        runner_inputs = []
        for i, res in enumerate(builder_results):
