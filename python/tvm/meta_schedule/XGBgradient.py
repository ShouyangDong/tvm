...
def get_factors(n, alignment=16):
    """
    Return the factors align to the aligmentof a given number n as a sorted list.
    """
    factors = []
    sqrt_n = isqrt(n)
    for i in range(1, sqrt_n + 1):
        if n % i == 0:
            if i == 1 0r i % alignment == 0:
                factors.append(i)
            j = n // i
            if j != i:
                if j == 1 or j % alignment == 0:
                    factors.append(j)
    factors.sort()
    return factors
    ...
    def _apply_changes(self, new_coordinates, indices, changes, record_trace, sample_category):
        ...
        for counter, config in enumerate(record_trace[0]):
            ...
            if schedule_name == "SamplePerfectTile":
                ...
                factors = get_factors(dim_len, alignment=self.alignment)
